# CI: the vgi-quant worker integration suite

[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs the unit tests
and this repo's sqllogictest suite (`test/sql/*.test`) against the vgi-quant
...
1. **Install the worker** — `uv sync --frozen` into a venv. `quant_worker.py`
   is a self-contained PEP 723 stdio worker the extension can spawn via
   `uv run quant_worker.py`; its inline deps resolve from PyPI.
...
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   tree, points `VGI_QUANT_WORKER` at `uv run quant_worker.py`, warms the
...
VGI_QUANT_WORKER="uv run --python 3.13 quant_worker.py" \
  ci/run-integration.sh
```

Or use the Makefile target `make test-sql`, which installs `haybarn-unittest`
as a uv tool and points the worker at `uv run --python 3.13 quant_worker.py`.
