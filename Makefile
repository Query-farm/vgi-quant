# vgi-quant — dev and test targets.
#
# Usage:
...
#
# test-sql is self-contained: it points VGI_QUANT_WORKER at the worker run as a
...
WORKER_STDIO    ?= uv run --python 3.13 quant_worker.py
